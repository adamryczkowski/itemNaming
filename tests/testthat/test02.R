...
testthat::test_that("Simple test for 5th step", {
  ...
  testthat::expect_equal(step_5_display_single_item(value = 123456789, properties=list()), "123 456 789")
  testthat::expect_equal(step_5_display_single_item(value = 1234567890123456789, properties=list()), "1 234 568 000 000 000 000")
...
#Displays a single item with the list of attributes
  step_4_display_item<-step_4_display_item_gen(txt_attribute_separator=', ', txt_attribute_separator_last=' i ',
  ...
  testthat::expect_equal(out, 'Mikołaj (`hue`: `Zielony`, `wzrost`: 135 i `waga`: 30)')
})
...
testthat::test_that("Simple test for 3th step", {
  ...
  step_3_comma<-step_3_comma_gen(txt_separator=', ', txt_separator_last = ' oraz ', display_item_fn=step_4_display_item)
  ...
  testthat::expect_equal(out, "Mikołaj (`hue`: `Zielony`, `wzrost`: 135 i `waga`: 30), `Wika` (`NA`, 157 i 83), `Adam` (`NA`, 171 i 75) oraz Sofia (`Czerwony`, 87 i NA)")
...
testthat::test_that("Simple test for 2nd step", {
  ...
  step_2_ellipsis<-step_2_ellipsis_gen(item_ellipsis=item_ellipsis_fn_pl,
                                       max_explicit_items_count=3, number_of_elements_around_ellipsis=c(2,1),
                                       step_3_comma)
  item_numeral<-Polish_numeral$new(mianownik='członek rodziny', dopelniacz='członków rodziny', lmnoga='członkowie rodziny',
...
#  debugonce(step_2_ellipsis)
  ...
  testthat::expect_equal(out, "Mikołaj (`hue`: `Zielony`, `wzrost`: 135 i `waga`: 30) oraz `Wika` (`NA`, 157 i 83), członek rodziny więcej, Sofia (`hue`: `Czerwony`, `wzrost`: 87 i `waga`: NA)")
})
...
testthat::test_that("Simple test for 1st step", {
  ...
  out<-step_1_inversion(items_df, all_except_infix = all_except_infix_fn_pl, item_numeral=item_numeral,
                        threshold_for_inversion=0.1, step_2_ellipsis, included_column_name='__included')



  #  debugonce(step_2_ellipsis)
  testthat::expect_equal(out, "wszyscy członkowie rodziny poza: `Adam` (`hue`: `NA`, `wzrost`: 171 i `waga`: 75)")
})
